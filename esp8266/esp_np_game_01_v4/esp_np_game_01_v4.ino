...
// 強力子彈顏色 (單顆粉紅色)
const uint32_t POWER_BULLET_COLOR   = Adafruit_NeoPixel::Color(50, 0, 50); 
// 準備燈顏色
...
void drawLights() {
  ...
  // 2. 覆蓋繪製子彈 - 共用繪製邏輯
  int currentBulletPos = -1;
  uint32_t colorToDraw = 0;

  if (isPowerShotActive && powerBulletPos != -1) {
    // 如果強力子彈啟動且正在飛行，使用強力子彈的位置和顏色
    currentBulletPos = powerBulletPos;
    colorToDraw = POWER_BULLET_COLOR;
  } else if (!isPowerShotActive && bulletPos != -1) {
    // 如果是標準子彈啟動且正在飛行，使用標準子彈的位置和顏色
    currentBulletPos = bulletPos;
    colorToDraw = BULLET_COLOR;
  }

  if (currentBulletPos != -1 && currentBulletPos < NUMPIXELS) {
    // 實際繪製子彈 (共用這一行)
    pixels.setPixelColor(currentBulletPos, colorToDraw);
  }
